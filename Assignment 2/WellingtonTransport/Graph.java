    ...
    private void createAndConnectEdges() {
        ...
        for (Line line : lines) {
           for (int i = 0; i < line.getStops().size() - 1; i++) {
                Stop stop1 = line.getStops().get(i);
                Stop stop2 = line.getStops().get(i + 1);
                double time = line.getTimes().get(i + 1) - line.getTimes().get(i);     //calculate time from stop1 to stop2
                double dist = stop1.distanceTo(stop2);     //calculate distance from stop1 to stop2
                Edge edge = new Edge(stop1, stop2, line.getType(), line, time, dist);
                edges.add(edge);
                stop1.addForwardEdge(edge);     //add edge to stop1's forward edge
                stop2.addBackwardEdge(edge);    //add edge to stop2's backward edge
           }
        }
    ...
    public void computeNeighbours(){
        for (Stop stop : stops) {
            for (Edge edge : stop.getForwardEdges()) {
                stop.addNeighbour(edge.toStop());               //adding the toStop of the edge to the neighbour of the stop
            }
            for (Edge edge : stop.getBackwardEdges()) {
                stop.addNeighbour(edge.fromStop());             //adding the fromStop of the edge to the neighbour of the stop
            }
        }
    }
    ...
    public void recomputeWalkingEdges(double walkingDistance) {
        ...
        for (Stop stop1 : stops) {
            for (Stop stop2 : stops) {
                if (stop1.distanceTo(stop2) <= walkingDistance && stop1 != stop2) {                                 //if the distance between stop1 and stop2 is less than walkingDistance
                    double time = stop1.distanceTo(stop2) / Transport.WALKING_SPEED_MPS;                                                    //calculate time from stop1 to stop2
                    Edge edge = new Edge(stop1, stop2, Transport.WALKING, null, time, stop1.distanceTo(stop2));
                    stop1.addForwardEdge(edge);     //add edge to stop1's forward edge
                    stop2.addBackwardEdge(edge);    //add edge to stop2's backward edge
                    edges.add(edge);
                    count++;
